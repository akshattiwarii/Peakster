...
export function HeroSection() {
    ...
    // Search handlers refactored to specific functions to avoid useEffect loops
    const handleDestinationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (value.length > 2) {
            const delayDebounceFn = setTimeout(async () => {
                try {
                    const response = await fetch(`https://photon.komoot.io/api/?q=${encodeURIComponent(value)}&limit=10`);
    ...
    return (
        <>
            ...
            {/* Toast Notification */}
            {message && (
                <div className="fixed bottom-10 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 z-[100] animate-in slide-in-from-bottom-5 fade-in duration-300 bg-red-500 text-white">
